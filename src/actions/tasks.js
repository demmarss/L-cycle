import { createTask, createAssignLgroup, createRemoveAssignLgroup, addingAudioResponseToTask, 
  ...
  gettingTask, receivingTasks, addingScoreHistoryToTasks, deletingTask, createTaskWithImages, createTaskMainInfo } from '../service/api'
...
// handler for creating task
  ...
  // handler for creating task
  export function handleCreateTaskWithImages(formData) {
    return (dispatch, getState) => {
      const { authedUser } = getState();
      const { token } = authedUser? authedUser:{token:""}
    return createTaskWithImages({
      formData,
      token,
       })
      .then(({task}) => {
